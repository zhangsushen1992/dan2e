...
class DAN2Regressor(object):
    ...
    def add_layer(self, X, y, isFirstLayer, f_k=0):
        if isFirstLayer:
            ...
            self.lin_predictions = f_k
            mu=1
            Xn = self.build_X1(f_k, self.alpha)
            lr = LinearRegression(fit_intercept=True).fit(Xn, y)
            A = lr.coef_[0]
            a = lr.intercept_
            f_k = lr.predict(Xn)
    ...
    def add_layer_multihead(self,X,y,head_number,f_k):
        ...
        coef_ = np.insert(coef_, 0, a)
        coef_ = np.insert(coef_, 0, mu)
        self.multi_logging(coef_,head_number)
        return f_k


    def predict_shared(self, X_test):
        X = X_test
        m = X.shape[0]
        alpha = self.compute_alpha(X)
        f_k = self.lin_predictor.predict(X)
    ...
    def predict_multihead(self, X_test,shared_f_k, head_number):
        X=X_test
        m=X.shape[0]
        alpha = self.compute_alpha(X)
        f_k = shared_f_k
