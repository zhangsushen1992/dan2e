"""
Alter non-linear functions: to Bessel
"""
import numpy as np
import pickle
...
class DAN2Regressor(object):
    ...
    def f(self, x):
        ...
        Xn = a + A[0]*f + A[1]*jv(x*alpha,0) + A[2]*jv(x*alpha,0)
    ...
    def compute_alpha(self, X):
        ...
        return np.random.rand(len(acos),1)
    ...
    def build_X1(self, f, alpha):
        return np.hstack((f, jv(alpha,0), jv(alpha,0)))
    ...
    def build_Xn(self, f, A, alpha, mu):
        ...
        if A is None and mu is None:
            X = np.hstack((f, jv(alpha,0), jv(alpha,0)))
            A = LinearRegression(fit_intercept=True).fit(X, y)

        return np.hstack((A[0]*f, A[1]*jv(alpha,mu), A[2]*jv(alpha,mu)))
    ...
    def predict(self, X_test):
        ...
        for coef_ in self.coef_:
            mu = coef_[0]
            if i == 0:
                X = np.hstack((f_k, jv(alpha*mu,0), jv(alpha*mu,0)))
                f_k = self._activation_function(X, coef_[1:])
                f_k = f_k.reshape(m,1)
            else:
                X = np.hstack((prev_coef_[2]*f_k, prev_coef_[3]*jv(alpha*mu,0), prev_coef_[4]*jv(alpha*mu,0)))
                f_k = self._activation_function(X, coef_[1:])
                f_k = f_k.reshape(m,1)
