...
class DAN2Regressor(object):
    ...
    def serial_logging(self, coef_, serial_number):
        if self.serial_coef_[serial_number] is None:
            self.serial_coef_[serial_number] = coef_.reshape(1,5)
        else:
            self.serial_coef_[serial_number]=np.vstack((self.serial_coef_[serial_number],coef_))
    ...
    def predict(self, X_test):
        ...
        serial_outputs = []
        for serial_number in range(0,OUTPUTS_NUMBER):
            i = 0

            for serial_coef_ in self.serial_coef_[serial_number]:
                mu = serial_coef_[0]
                if i == 0:
                    X = np.hstack((f_k, np.cos(alpha*mu), np.sin(alpha*mu)))
                    f_k = self._activation_function(X, serial_coef_[1:])
                    f_k = f_k.reshape(m, 1)
                else:
                    X = np.hstack((prev_serial_coef_[2]*f_k, prev_serial_coef_[3]*np.cos(alpha*mu), prev_serial_coef_[4]*np.sin(alpha*mu)))
                    f_k = self._activation_function(X, prev_serial_coef_[1:])
                    f_k = f_k.reshape(m, 1)
                prev_serial_coef_ = serial_coef_
                i += 1
            serial_outputs.append(f_k)

        return f_k, serial_outputs
    ...
    def serial(self, X, y, serial_depth, serial_number, prev_f_k):
        m = X.shape[0]
        alpha=self.compute_alpha(X)
        f_k, i = prev_f_k,1
        mu = np.random.random()
        while i <= serial_depth:
            if i == 1:
                Xn = self.build_X1(f_k, alpha)
                lr = LinearRegression(fit_intercept=True).fit(Xn, y)
                A = lr.coef_[0]
            ...
            mse = self.mse(f_k, y, m)
            serial_coef_ = A.reshape((1, 3))
            serial_coef_ = np.insert(serial_coef_, 0, a)
            serial_coef_ = np.insert(serial_coef_, 0, mu)
            self.serial_logging(serial_coef_,serial_number)
            i+=1
